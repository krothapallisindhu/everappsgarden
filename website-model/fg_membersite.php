 ...
 */
require_once("dbaccess/class.phpmailer.php");
...
class FGMembersite {
    ...
    function FGMembersite() {
        $this->sitename = 'http://www.everappsgarden.com';
    ...
    function Login($appType) {
        ...
        if (!$this->CheckLoginInDB($username, $password, $type, $appType)) {
            return false;
        }

        $_SESSION[$this->GetLoginSessionVar()] = $username;
        //$_SESSION[$this->GetLoginSessionVar()] = $type;
    ...
    function CheckLoginInDB($username, $password, $type, $appType) {
        if ($appType == 'web') {
            if (!$this->DBLogin()) {
        ...
        $type = $this->SanitizeForSQL($type);

        $pwdmd5 = md5($password);
        $databse = new DBHandler();
        $qry = "Select name, email,type from tb_users where username='$username' and password='$pwdmd5' and confirmcode='y' and type ='$type'";
        if ($appType == 'api') {
            $result = $databse->executeSql($qry);
            return $result;
        } else {
    ...
    function CollectRegistrationSubmission(&$formvars) {
        ...
        $formvars['role'] = $this->Sanitize('admin');
    ...
    public function getDriverDetails($appType) {

        $driverCollection = new GenericCollection($appType);
        $databse = new DBHandler();

        $sql = sprintf("Select u.id_user,u.name,u.type,u.role,u.phone_number,u.username,ui.lattitude,ui.longitude,u.confirmcode from tb_users u left join tb_userinfo ui on u.id_user=ui.id_user Where u.role='driver'");
        if ($appType == 'api') {
            ...
            while ($row = mysql_fetch_assoc($result)) {
                ...
                $driverCollection->addObject($i, $driverDetails);
                $i = $i + 1;
            }
    ...
    public function getUserDetails($appType) {
        $myCollection = new GenericCollection($appType);
        $databse = new DBHandler();

        $sql = sprintf("Select u.id_user,u.name,u.type,u.role,u.phone_number,u.username,ui.lattitude,ui.longitude,u.confirmcode from tb_users u left join tb_userinfo ui on u.id_user=ui.id_user Where u.role='user'");
        if ($appType == 'api') {
            $result = $databse->executeSql($sql);
            return $result;
        } else {
    ...
    public function getCompanyDetails($appType) {
        $myCollection = new GenericCollection($appType);
        $databse = new DBHandler();

        $sql = sprintf("Select u.id_user,u.name,u.type,u.role,u.phone_number,u.username,ui.lattitude,ui.longitude,u.confirmcode from tb_users u left join tb_userinfo ui on u.id_user=ui.id_user Where u.type='company'");
        if ($appType == 'api') {
            $result = $databse->executeSql($sql);
            return $result;
        } else {
    ...
    public function getEnquiryDetails($appType) {
        $myCollection = new GenericCollection($appType);
        $databse = new DBHandler();
        $sql = sprintf("Select b.booking_id,c.cab_name,u.name,b.from_address,b.to_address,b.passenger_number,b.laugage,b.description,b.booking_date from tb_booking b Left Join tb_cabs c on b.cab_id=c.cab_id left join tb_users u on b.driver_id=u.id_user ");
        if ($appType == 'api') {
            $result = $databse->executeSql($sql);
            return $result;
        } else {
