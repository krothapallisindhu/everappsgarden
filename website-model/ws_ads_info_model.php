...
class ws_ads_info_model {
    ...
    public function getAdDetails($aAppType, $aAppname) {
    ...
    public function getAdsList() {
        ...
        //$query = "SELECT * FROM apps_info;";
        $query = "SELECT * FROM " . ADS_INFO_TABLE_NAME . ";";
        ...
        while ($row = mysql_fetch_array($resultedrows)) {
            ...
            //Creating dataobject from the database result
            $ad_details = new eag_ads_info();
            $ad_details->setID($row['id']);
            $ad_details->setType($row['type']);
            //$app_details->setAppStoreURL($row['store_url']);
            // $app_details->setImageURL($row['image_url']);
            //$app_details->setStartDate($row['start_date']);
            //$app_details->setEndDate($row['end_date']);
            
            //Adding data object to collection object
            $dataObjectsArray->addObject($i, $ad_details);
            $i = $i + 1;
        }
